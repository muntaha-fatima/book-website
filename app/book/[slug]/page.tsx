...
export default function BooksPage() {
  ...
  // Fetch books safely and normalize response
  const fetchBooks = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (!res.ok) {
        let errMsg = `Failed to fetch books: ${res.status}`;
        try {
  ...
  const suggestionsMemo = useMemo(() => {
    ...
    return books
      ...
      .map((book) => {
        const titleSafe = book.title || book.titleAr || "Untitled";
        let type: "Book" | "Author" | "Category" = "Book";
        if (book.author?.toLowerCase().includes(term)) type = "Author";
        else if (book.category?.toLowerCase().includes(term)) type = "Category";
        return {
          title: titleSafe,
          type,
  ...
  const filteredBooks = useMemo(() => {
    return books.filter((book) => {
      ...
      const matchesSearch =
        ...
        (book.titleAr?.includes(searchTerm) ?? false) ||
        (book.descriptionAr?.toLowerCase().includes(term) ?? false) ||
        (book.category?.toLowerCase().includes(term) ?? false);
      const matchesCategory = selectedCategory === "All" || book.category === selectedCategory;
      return matchesSearch && matchesCategory;
    });
  ...
  const shareBook = (book: Book | null) => {
    ...
    try {
      const slug = (book._id ?? book.title ?? "book")
        .toString()
        .toLowerCase()
        .replace(/\s+/g, "-");
  ...
  return (
    <main className="min-h-screen bg-cream-100 py-8 px-4 sm:px-6 lg:px-8">
      ...
      <section className="relative bg-gradient-to-br from-teal-700 to-teal-900 text-white py-28 px-4 mb-16 overflow-hidden">
        <div className="absolute inset-0 bg-[url('/books-bg.jpg')] bg-cover bg-center opacity-15 mix-blend-overlay" />
        <div className="relative max-w-7xl mx-auto text-center">
          <h1 className="text-5xl sm:text-7xl font-extrabold english-text mb-4 tracking-tighter drop-shadow-md">
            Bettywrites
          </h1>
          <h2 className="text-3xl sm:text-5xl font-bold arabic-text mb-8" dir="rtl">
            بيتي رايتس
          </h2>
                ...
                {suggestions.map((suggestion, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <Badge className="ml-3 bg-teal-100/80 text-teal-700 text-xs font-semibold tracking-wide">
                        {suggestion.type}
                      </Badge>
                    </div>
                  </div>
      ...
      <section className="max-w-7xl mx-auto py-12 space-y-12">
          ...
          <div className="flex gap-4 whitespace-nowrap px-4 sm:px-0">
            {categories.map((category) => (
              <Button
                ...
                className={`flex items-center gap-3 px-6 py-3 rounded-full btn-neumorphic transition-all duration-300 ${
                  selectedCategory === category.name ? category.color + " text-white shadow-lg" : "bg-white/80 border-teal-200/50 text-teal-700 hover:bg-teal-50/80 hover:shadow-md"
                }`}
            ...
            {selectedCategory !== "All" || searchTerm ? (
              <Button
                ...
                className="bg-white/80 border-rose-200 text-rose-600 hover:bg-rose-50 btn-neumorphic rounded-full px-6 py-3"
        ...
        {error ? (
          <div className="text-center py-28 bg-white/90 border border-teal-100/30 rounded-3xl shadow-xl backdrop-blur-md">
            ...
            <Button onClick={fetchBooks} className="bg-teal-600 hover:bg-teal-700 text-white rounded-full btn-neumorphic px-8 py-3 font-semibold" aria-label="Retry loading books">Retry</Button>
          </div>
        ) : loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {Array.from({ length: 9 }).map((_, index) => (
              <div key={index} className="bg-white/90 border border-teal-100/30 rounded-3xl p-6 animate-pulse shadow-lg">
                <div className="h-72 bg-teal-100/30 rounded-2xl mb-4" />
                <div className="h-6 bg-teal-100/30 rounded w-3/4 mb-2" />
                <div className="h-5 bg-teal-100/30 rounded w-1/2 mb-4" />
                <div className="h-10 bg-teal-100/30 rounded-2xl" />
              </div>
            ))}
          </div>
        ) : filteredBooks.length > 0 ? (
                ...
                >
                  ...
                  <CardContent className="p-6 pt-0">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {book.price && <span className="text-base font-bold text-teal-600 english-text">${book.price.toFixed(2)}</span>}
                    </div>
          ...
          <div className="text-center py-28 bg-white/90 border border-teal-100/30 rounded-3xl shadow-xl backdrop-blur-md">
            ...
            <Button onClick={clearFilters} className="mt-8 bg-teal-600 hover:bg-teal-700 text-white rounded-full btn-neumorphic px-8 py-3 font-semibold" aria-label="Clear search and filters">Clear Search</Button>
          </div>
        )}
            ...
            <DialogHeader className="sticky top-0 z-10 bg-white/90 border-b border-teal-100/30 p-6 shadow-lg">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  <Button variant="ghost" size="sm" onClick={() => document.documentElement.requestFullscreen()} aria-label="Toggle fullscreen" className="hover:bg-teal-50 rounded-full">
                    <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4h4m8 0h4v4m-4 8h4v4m-8-4v4h-4" />
                    </svg>
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => document.body.classList.toggle("dark")} aria-label="Toggle dark mode" className="hover:bg-teal-50 rounded-full">
                    <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                    </svg>
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => shareBook(selectedBook)} aria-label="Share book" className="hover:bg-teal-50 rounded-full">
...
/>
                  ...
                  <div className="mt-6 flex items-center gap-2">
                    {typeof selectedBook?.rating === "number" ? (
                      <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} className={`w-7 h-7 ${i < Math.round(selectedBook.rating ?? 0) ? "text-amber-400 fill-amber-400" : "text-slate-200"}`} />
                        ))}
                  ...
                  <p className="mt-4 text-base text-slate-600 english-text font-medium">Category: {selectedBook?.category || "Unknown"}</p>
                  <p className="text-base text-slate-600 arabic-text font-medium" dir="rtl">الفئة: {selectedBook?.categoryAr || "غير معروف"}</p>
                  {selectedBook?.price && <p className="mt-2 text-lg font-bold text-teal-600 english-text">${selectedBook.price.toFixed(2)}</p>}
                </div>

                <div className="md:col-span-3">
                  <h3 className="text-2xl font-bold english-text mb-4">Overview</h3>
                  <p className="text-base text-slate-700 english-text mb-6">{selectedBook?.description || "No description available."}</p>
                  <h4 className="text-xl arabic-text mb-3" dir="rtl">تفاصيل</h4>
                  <p className="text-base arabic-text" dir="rtl">{selectedBook?.descriptionAr || selectedBook?.description || "لا يوجد وصف متاح"}</p>
                </div>
              </div>
            </div>
          </DialogContent>
